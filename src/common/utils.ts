...
export class Utils {
	...
	static coinSelection(utxos: Utxo[], requestedValue: Value, config: any, maxInputCount: number, checkChangeMinUtxo = true): { selection: Utxo[], change?: Value } {
		let selection: Utxo[] = [];
		...
		const address = selection[0].address;
		if (checkChangeMinUtxo) {
			const minUtxo = Seed.getMinUtxoValueWithAssets(address, [...change.assets.values()], null, null, config, 'hex');
			if (0 < change.coin && change.coin < minUtxo) {
				try {
					const { selection: additional } = Utils.coinSelection(
						inputs,
						new Value(minUtxo - change.coin),
						config,
						maxInputCount - selection.length,
						false
					);
					for (let utxo of additional) {
						selection.push(utxo);
						change.add(new Value(Number(utxo.value), utxo.assets));
					}
				} catch (_) { }
			}
		}
		const assets = [...change.assets.values()];
		const minUtxo = Seed.getMinUtxoValueWithAssets(address, assets, null, null, config, 'hex');
		if (0 < change.coin && change.coin < minUtxo) {
			change.addAda(-change.coin);
		}
		if (change.coin == 0 && assets.length > 0) {
			throw new Error(`Insufficient UTxO balance`);
		}
	...
	static async getUtxos(ledger: DbClient, address: string, size: number, order: string, pageToken: string, encoding: BufferEncoding = 'utf8'): Promise<{ utxos: Utxo[], cursor: string | null }> {
		...
		return {
			utxos: encoding != 'utf8' ? items.map(utxo => ({
				...utxo,
