import { fromValue, createMap, forMember, ignore, mapDefer, mapFrom } from '@automapper/core';
import { AutomapperProfile, InjectMapper } from '@automapper/nestjs';
import type { Mapper } from '@automapper/core';
import { Injectable } from '@nestjs/common';
import { CostModel, EpochParameters, Metadata } from '@tangocrypto/tango-ledger';
...
export class EpochParametersProfile extends AutomapperProfile {
  ...
  get profile() {
    return (mapper: Mapper) => {
      createMap<EpochParameters, EpochParametersDto>(mapper, 'EpochParameters', 'EpochParametersDto',
        ...
        forMember(dest => dest.cost_model, mapDefer<EpochParameters>(src => src.cost_model ? fromValue(mapper.map<CostModel, CostModelDto>(src.cost_model, 'CostModel', 'CostModelDto')) : ignore())),
        forMember(dest => dest.block_id, ignore()),
      );

      createMap<CostModel, CostModelDto>(mapper, 'CostModel', 'CostModelDto',
