...
export class AssetProfile extends AutomapperProfile {
  ...
  get profile() {
    return (mapper: Mapper) => {
      createMap<Asset, AssetDto>(mapper, 'Asset', 'AssetDto',
        ...
        forMember(dest => dest.metadata, mapDefer<Asset>(src => src.metadata ? fromValue(mapper.mapArray<Metadata, MetadataDto>(src.metadata, 'Metadata', 'MetadataDto')) : ignore())),
      );

      createMap<AssetOwner, AssetOwnerDto>(mapper, 'AssetOwner', 'AssetOwnerDto',
